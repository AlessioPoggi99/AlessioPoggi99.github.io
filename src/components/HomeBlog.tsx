...
export default function HomeBlog({ posts }) {
  useEffect(() => {
    const imgContainerArr = document.querySelectorAll(
      "#blog .card .blog-img-container",
    );
    imgContainerArr.forEach((container) => {
      const imgElement = container.firstChild?.firstChild as HTMLElement;
      if (
        imgElement &&
        imgElement.offsetHeight < (container as HTMLElement).offsetHeight
      )
  ...
  const handleMouseEnter = (e) => {
    ...
    const parentHeight =
      e.target.parentNode.parentNode.getBoundingClientRect().height;
    ...
    <>
      <div className="container">
        {posts.map((post) => (
          <MotionHorizontal
            ...
            immidiate={false}
            key={post.id}
            _className="card bg-[color:var(--border)] p-4 rounded-lg"
          >
            ...
            <div className="blog-text-container text-justify flex flex-col justify-start pl-4">
              <a href={"/blog/" + post.slug}>
                ...
                <FormattedDate
                  ...
                  dateClass="mt-3 text-xs opacity-50 flex justify-center"
                />
                <p className="mt-5 line-clamp-5 text-sm leading-6">
      ...
      >
        <Button
          ...
          blank={false}
          buttonClass={
            "text-sm font-semibold leading-6 text-[var(--decoration)] cursor-pointer uppercase"
          }
        >
          All posts &rarr;
        </Button>
